...
def create_detail(todo_id: int, detail: DetailCreate, db = Depends(db_dependency), user = Depends(user_dependency)):
...
# Delete detail
@router.delete('/')
def delete_detail(todo_id: int, db = Depends(db_dependency), user = Depends(user_dependency)):
